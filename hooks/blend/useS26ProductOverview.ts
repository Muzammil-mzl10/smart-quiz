  ...
  const calculateTotalAmounts = useCallback(
    (recommendations: any[], supplement: any) => {
      ...
      return {
        ...
        calculatedAmount,
        vitexAgnusCastus: (
          supplement.baseAmounts.vitexAgnusCastus * Math.min(totalDoseCount, 1)
        ).toFixed(4),
        dongQuai: (
          supplement.baseAmounts.dongQuai * Math.min(totalDoseCount, 1)
        ).toFixed(4),
        vitaminB6: (
...
export default useS26ProductOverview;
