  ...
  const calculateTotalAmounts = useCallback(
    (recommendations: any[], supplement: any) => {
      ...
      return {
        ...
        calculatedAmount,
        milkThistle: (
          supplement.baseAmounts.milkThistle * Math.min(totalDoseCount, 1.5)
        ).toFixed(4),
        sAcetylLGlutathione: (
          supplement.baseAmounts.sAcetylLGlutathione *
          Math.min(totalDoseCount, 1.5)
        ).toFixed(4),
        reasons,
      };
...
export default useS12ProductOverview;
